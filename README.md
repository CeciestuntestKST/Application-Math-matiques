...
- **Leçons d'oral** : constituez des plans de leçon à partir de la bibliothèque de notions. Dans la section Notions, le bouton **Sélectionner** de la toolbar active le cochage des cartes (compteur, « Créer la leçon », « Vider ») ; la leçon s'ouvre alors dans la section **Leçons d'oral** : titre éditable, notions dans l'ordre voulu (↑ ↓ pour réordonner, × pour retirer), notions de même nom fusionnées affichées ensemble, mentions des notions devenues introuvables (cours modifié ou supprimé). Les leçons sont listées dans la sidebar de la section et **persistées dans les préférences** (`prefs.json` — jamais dans les `.tex`) ; elles survivent aux re-scans du dossier. L'export d'un fichier `.tex` compilable est prévu (TODO).
...
lib/
  ...
  lessons-model.js       Modèle des leçons d'oral (création, réordonnancement, résolution des notions) — Node pur, testé (npm test)
...
test/run-tests.js        Tests unitaires (npm test) — 33 tests, sans Electron
