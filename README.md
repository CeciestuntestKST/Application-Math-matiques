...
- **Notions** : toutes les notions de tous les cours sont regroupées **en haut de la section** (grille de cartes paginée, sous la barre de recherche). Filtres : recherche plein texte (titre, environnement, matière), **Nommées / Anonymes** et **matière**. Les notions sans titre reçoivent un nom générique numéroté (`Définition 1`, `Définition 2`…, selon l'environnement : `df` → Définition, `re` → Remarque, `ra` → Rappel, `dfprop` → Définition-Proposition, `prop` → Proposition, `tm` → Théorème, `not`/`nt` → Notation, `lm` → Lemme, `cor` → Corollaire, `ex` → Exemple, `exo` → Exercice). Les notions **nommées identiques** (même titre, toutes matières confondues) sont **fusionnées** : une seule carte dans la grille, plusieurs cases dans le même onglet à l'ouverture. Les **démonstrations** (`proof`/`proof*`/`demonstration`) ne s'affichent pas dans la liste : elles sont rattachées à la notion qu'elles suivent (ou à la notion de même nom) et apparaissent repliées sous la notion dans l'onglet ouvert. Chaque onglet affiche la notion compilée en LaTeX (KaTeX) avec les macros de `settings.tex`, un bouton **Copier le code LaTeX** et une bascule **Code source**. Le rendu est mis en cache : les clics restent instantanés même avec ~1500 notions.
