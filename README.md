...
| `lecons/*.tex` | Leçons d'oral créées dans l'app (section dédiée) : plan en LaTeX + métadonnées (numéro, titre) en en-tête. Exclu du scan des notions. |
...
- **Leçons d'oral** : rédigez vos plans de leçons d'oral en LaTeX, directement dans l'app. L'accueil de la section liste vos leçons en cartes (numéro, titre, date de modification) ; **« + Créer une leçon »** ouvre un formulaire numérotation + titre. L'éditeur offre : un **champ LaTeX libre** (le plan de la leçon, sauvegardé automatiquement dans un petit fichier `.tex` créé dans le sous-dossier `lecons/` du dossier de cours), et un **panneau latéral « Bibliothèque de notions »** avec recherche — un clic sur une notion **insère son code LaTeX** à l'emplacement du curseur dans la leçon. Chaque leçon est donc un vrai fichier `.tex` (numéro et titre stockés en en-tête sous forme de commentaire `% lesson-meta:`), éditable aussi hors de l'app ; le dossier `lecons/` est exclu du scan des notions et du rechargement automatique. Le titre et le numéro sont modifiables (le fichier est renommé automatiquement), et « Supprimer » efface le fichier avec confirmation.
- **Développements** : section dédiée aux développements potentiels pour l'oral. Même fonctionnement que les leçons : accueil en cartes (titre, nombre de leçons compatibles, date), **« + Créer un développement »** → formulaire de titre, **éditeur LaTeX libre** avec autosauvegarde dans un petit fichier `.tex` du sous-dossier `developpements/` du dossier de cours. Le panneau latéral réunit deux outils : **« Leçons compatibles »** — cases à cocher listant vos leçons d'oral (le lien survit au renommage des fichiers grâce à un identifiant stable) — et **« Bibliothèque de notions »** avec recherche et insertion du LaTeX au clic. Les métadonnées (titre, leçons compatibles) sont stockées en en-tête sous forme de commentaire `% dev-meta:` ; le dossier `developpements/` est exclu du scan des notions et du watcher.
...
test/run-tests.js        Tests unitaires (npm test) — 46 tests, sans Electron
