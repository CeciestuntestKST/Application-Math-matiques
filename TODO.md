...
### Fait
- [x] **Oral — blocs notions compacts et anti-troncature dans les aperçus** : dans les aperçus intégrés (plans et développements) de la section Oral, les blocs notions adoptent un en-têtre minimal « Type de notions : ‹type› » (badge et titre supprimés) et un corps très dense ; les formules larges ne sont plus coupées : `min-width: 0` sur les blocs et défilement horizontal local des `katex-display`. Les aperçus des sections Plans et Développements gardent l'en-tête complet (badge + titre).
- [x] **Oral — aperçu intégré des développements** : dans la vue détail d'un numéro, un clic sur un développement potentiel ouvre **son aperçu directement dans le panneau droit de la section Oral** (sommaire + rendu fiche, même traitement que les plans) au lieu de basculer vers la section Développements ; boutons **flèche retour** (revenir à la liste) et **crayon** (ouvrir l'élément dans son éditeur) en icônes SVG sur les deux panneaux (plans et développements).
