...
### Fait
- [x] **Oral — blocs notions compacts et anti-troncature dans les aperçus** : dans les aperçus intégrés (plans et développements) de la section Oral, badge et titre des blocs notions sont supprimés ; le type de la notion ouvre directement le corps en un seul flux « ‹Type› : ‹corps› » très dense. Les formules larges ne sont plus coupées : `min-width: 0` sur les blocs, `overflow-x: auto` et `text-align: safe center` sur les `katex-display` (centrées si elles tiennent, alignées à gauche et défilables si elles débordent). Les aperçus des sections Plans et Développements gardent l'en-tête complet (badge + titre).
