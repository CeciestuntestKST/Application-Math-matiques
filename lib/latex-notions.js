...
const SECTION_LEVELS = { chapter: 0, section: 1, subsection: 2, subsubsection: 3 };
...
function extractSections(source) {
  ...
  while ((match = re.exec(cleaned)) !== null) {
    ...
    const level = SECTION_LEVELS[command];
    const optional = match[3] !== undefined ? match[3].trim() : null;
    const braced = match[4] !== undefined ? match[4].trim() : null;
    const title = braced !== null && braced !== '' ? braced : (optional !== null ? optional : '');
    if (title) {
      sections.push({ level, title, position: match.index, end: match.index + match[0].length });
    }
  }
  return sections;
}
